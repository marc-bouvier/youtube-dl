...
add-apt-repository -y ppa:ondrej/php && \
  ...
  apt-get -y update && \
  apt-get -y upgrade && \
  apt-get -y install supervisor sudo wget git apache2 php-xdebug libapache2-mod-php php-mysql pwgen php-apcu php-gd php-xml php-mbstring php-gettext zip unzip php-zip curl php-curl python3-pip ffmpeg mkvtoolnix php7.1-mcrypt aria2 && \
