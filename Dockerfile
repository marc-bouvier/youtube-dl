...
RUN add-apt-repository -y ppa:ondrej/php && \
  ...
  apt-get update && \
  apt-get -y upgrade && \
  apt-get -y install supervisor wget git apache2 php-xdebug libapache2-mod-php php-mysql pwgen php-apcu php1.2-mcrypt php-gd php-xml php-mbstring php-gettext zip unzip php-zip curl php-curl python3-setuptools ffmpeg mkvtoolnix aria2 && \
